...
def appointment(): 
    ...
    return render_template('appointment.html',query=query)


@app.route("/modify/<string:pid>",methods=['POST','GET'])
@login_required
def modify(pid):
    ...
    return render_template('modification.html',posts=posts)
...
def pf():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `feedback` (`email`,`feedback`) VALUES ('{email}','{feedback}')")
...
@login_required
def details():
    posts=db.engine.execute("SELECT * FROM `triggers`")
...
def search():
    if request.method=="POST":
        ...
        if name:
            ...
            flash("Doctor is Available","primary")
        else:
